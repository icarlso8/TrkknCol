...
export async function mostrarGaleriaLogos(canvas) {
  ...
  const response = await fetch("../../Anunciante/D1/json/logos.json");
  const logos = await response.json();
  
  logos.forEach(logo => {
    const img = document.createElement("img");
    img.src = "../../Anunciante/D1/assets/logos/" + logo.nombreArchivo;
...
export async function mostrarGaleriaIconos(canvas) {
  ...
  const response = await fetch("../../Anunciante/D1/json/icons.json");
  const iconos = await response.json();

  iconos.forEach(icono => {
    const img = document.createElement("img");
    img.src = "../../Anunciante/D1/assets/icons/" + icono.nombreArchivo;
...
export async function generarCreatividadesConFondos(canvas, audiencia, factorId, opcionId, tamañoId, producto, callback) {
  ...
  const rutaBase = `../../Anunciante/D1/assets/fondos/${audiencia}/${factorId}/${opcionId}/${tamañoId}`;
  ...
  for (const archivo of fondos) {
    ...
    const nombreCreatividad = `OmniAdsAI_D1_${audiencia}_${opcionId}_${tamañoId}_${String(window.totalGeneradas).padStart(4, "0")}.png`;
